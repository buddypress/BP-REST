...
class BP_REST_Group_Membership_Request_Endpoint extends WP_REST_Controller {
	...
	/**
	 * Checks if a given request has access to accept or reject a group membership request.
	 *
	 * @since 0.1.0
	 *
	 * @param WP_REST_Request $request Full details about the request.
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( true === $retval && bp_current_user_can( 'bp_moderate' ) ) {
			...
			if ( true === $retval && ( ! groups_is_user_admin( $user->ID, $group->id ) || ! groups_is_user_mod( $user->ID, $group->id ) ) ) {
				$retval = new WP_Error( 'bp_rest_group_membership_request_update_failed',
					__( 'User cannot have request membership issued to this group.', 'buddypress' ),
					array(
						'status' => 500,
					)
	...
	/**
	 * Get the query params for pending group membership request.
	 *
	 * @since 0.1.0
	 *
	 * @return array
	...
	public function get_update_params() {
		...
		$params['action'] = array(
			'description'       => __( 'Action to accept or reject pending group membership request.', 'buddypress' ),
			'default'           => 'accept',
			'type'              => 'string',
